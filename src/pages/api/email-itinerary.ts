...
const DEST_NAMES: Record<string, string> = {
  'anchorage': 'Anchorage', 'fairbanks': 'Fairbanks', 'juneau': 'Juneau',
  'denali-national-park': 'Denali National Park', 'glacier-bay': 'Glacier Bay',
  'kenai-fjords': 'Kenai Fjords', 'sitka': 'Sitka', 'ketchikan': 'Ketchikan',
  'skagway': 'Skagway', 'seward': 'Seward', 'homer': 'Homer', 'kodiak': 'Kodiak',
  'valdez': 'Valdez', 'wrangell-st-elias': 'Wrangell-St. Elias',
  'katmai-national-park': 'Katmai National Park', 'nome': 'Nome',
  'utqiagvik': 'Utqiagvik', 'haines': 'Haines', 'talkeetna': 'Talkeetna',
  'whittier': 'Whittier', 'petersburg': 'Petersburg',
};
...
function buildAffiliateUrl(type: string | null | undefined, destination: string, description: string, tp?: TripParams): string {
  ...
  if (type === 'hotel') {
    ...
    const query = hotelName ? hotelName + ' ' + destName : destName + ' Alaska';
    let url = `https://www.booking.com/searchresults.html?ss=${enc(query)}&aid=2778866&label=discoveralaska`;
...
function buildHotelsComUrl(destination: string, description: string): string {
  ...
  const landingPage = encodeURIComponent(`https://www.hotels.com/Hotel-Search?destination=${encodeURIComponent(query + ' Alaska')}`);
...
function buildItineraryEmail(it: Itinerary, tp?: TripParams): string {
  ...
  for (const day of it.days) {
    ...
    for (const item of day.items) {
      ...
      if (item.affiliateSlotId && item.affiliateType) {
        ...
        if (item.affiliateType === 'hotel') {
          ...
          bookLink = ` <a href="${esc(url)}" style="display:inline-block;font-size:12px;font-weight:600;color:#0077B6;text-decoration:none;border:1px solid #0077B6;padding:2px 10px;border-radius:100px;margin-left:4px;white-space:nowrap;">Booking.com &rarr;</a> <a href="${esc(hotelsComUrl)}" style="display:inline-block;font-size:12px;font-weight:600;color:#1A2332;text-decoration:none;border:1px solid #1A2332;padding:2px 10px;border-radius:100px;margin-left:4px;white-space:nowrap;">Hotels.com &rarr;</a>`;
        } else {
          const label = item.affiliateType === 'tour' ? 'Book Tour' : item.affiliateType === 'transport' ? 'Book Transport' : 'Book This';
          bookLink = ` <a href="${esc(url)}" style="display:inline-block;font-size:12px;font-weight:600;color:#0077B6;text-decoration:none;border:1px solid #0077B6;padding:2px 10px;border-radius:100px;margin-left:4px;white-space:nowrap;">${label} &rarr;</a>`;
        }
      }
      ...
      itemsHtml += `
        <tr>
          ...
          <td style="padding:6px 0;vertical-align:top;">
            <span style="font-size:12px;font-weight:600;color:#0077B6;">${esc(item.time)}</span><br>
    ...
    daysHtml += `
      <div style="background:white;border-radius:12px;padding:20px;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,0.06);">
        <div style="font-size:11px;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;color:#0077B6;margin-bottom:4px;">Day ${day.dayNumber}</div>
  ...
  const budgetHtml = it.totalBudget ? `
    <div style="display:flex;justify-content:space-between;align-items:center;background:#FDF2F2;border-radius:10px;padding:12px 20px;margin-bottom:20px;">
      <span style="font-size:14px;font-weight:600;color:#0077B6;">Estimated Total</span>
...
<body style="margin:0;padding:0;background:#F5F0E8;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',system-ui,sans-serif;">
  <div style="max-width:600px;margin:0 auto;padding:32px 16px;">
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#0077B6,#FF6B35);border-radius:16px 16px 0 0;padding:28px 24px;text-align:center;">
      <h1 style="color:white;font-size:22px;margin:0 0 6px;">Your Alaska Itinerary</h1>
    ...
    <div style="background:#F5F0E8;border-radius:0 0 16px 16px;padding:24px 20px;">
      ...
      <div style="text-align:center;padding:24px 0 16px;">
        <a href="https://discoveralaska.info/plan/" style="display:inline-block;background:#0077B6;color:white;padding:14px 32px;border-radius:100px;text-decoration:none;font-weight:700;font-size:15px;">Generate Another Itinerary</a>
      </div>

      <p style="font-size:12px;color:#94A3B8;text-align:center;line-height:1.5;margin:0;">
        Generated by <a href="https://discoveralaska.info/" style="color:#0077B6;text-decoration:none;">Discover Alaska</a> AI Trip Planner<br>
        Real prices &middot; Real experience &middot; Authentic Alaska
      </p>
    </div>

    <!-- Footer -->
    <p style="color:#94A3B8;font-size:11px;text-align:center;margin:16px 0 0;line-height:1.5;">
      <a href="https://discoveralaska.info/legal/privacy/" style="color:#94A3B8;">Privacy Policy</a> &middot;
      <a href="https://discoveralaska.info/legal/affiliate-disclosure/" style="color:#94A3B8;">Affiliate Disclosure</a>
    </p>
  </div>
</body>
</html>`;
...
export async function POST(context: APIContext): Promise<Response> {
  ...
  try {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'Discover Alaska <hello@discoveralaska.info>',
        to: [email],
        subject: `Your Alaska Itinerary: ${it.title}`,
        html,
      }),
    });
