...
function extractDestsFromText(text: string): string[] {
  ...
  const themeMap: Record<string, string[]> = {
    'glacier': ['glacier-bay', 'kenai-fjords', 'valdez'],
    'bear': ['katmai-national-park', 'kodiak', 'denali-national-park'],
    'wildlife': ['denali-national-park', 'katmai-national-park', 'kenai-fjords'],
    'aurora': ['fairbanks', 'anchorage'],
    'northern lights': ['fairbanks', 'anchorage'],
    'fishing': ['homer', 'kodiak', 'seward', 'ketchikan'],
    'whale': ['juneau', 'sitka', 'glacier-bay', 'petersburg'],
    'gold rush': ['skagway', 'fairbanks', 'nome'],
    'history': ['sitka', 'skagway', 'nome'],
    'cruise': ['juneau', 'ketchikan', 'skagway', 'glacier-bay'],
    'food': ['anchorage', 'juneau', 'homer'],
    'seafood': ['homer', 'ketchikan', 'juneau', 'kodiak'],
    'family': ['anchorage', 'seward', 'talkeetna'],
    'honeymoon': ['sitka', 'glacier-bay', 'juneau'],
    'romantic': ['sitka', 'glacier-bay', 'homer'],
    'budget': ['anchorage', 'fairbanks', 'talkeetna'],
    'luxury': ['glacier-bay', 'katmai-national-park', 'kodiak'],
    'adventure': ['wrangell-st-elias', 'denali-national-park', 'valdez'],
    'hiking': ['denali-national-park', 'wrangell-st-elias', 'kenai-fjords'],
    'kayak': ['glacier-bay', 'whittier', 'valdez'],
    'culture': ['sitka', 'ketchikan', 'utqiagvik'],
    'arctic': ['utqiagvik', 'nome', 'fairbanks'],
    'mountain': ['denali-national-park', 'talkeetna', 'valdez'],
    'train': ['anchorage', 'seward', 'denali-national-park', 'fairbanks'],
    'ferry': ['juneau', 'sitka', 'ketchikan', 'haines'],
  };
  ...
  return ['anchorage', 'denali-national-park', 'juneau'];
}
...
function buildSystemPrompt(): string {
  return `You are the Discover Alaska AI Trip Planner. You generate detailed day-by-day itineraries for Alaska travel.
...
- Be specific: real place names, real prices, real transport options (car rental, rideshare, etc.)
- Tag hotel/tour/transport items with affiliateType and affiliateSlotId for future monetization
- affiliateSlotId format: "day{N}-{type}-{destination}" e.g. "day1-hotel-anchorage"
- Most Alaska trips require a rental car for inter-city travel
...
{
  "title": "string — trip title like '7-Day Denali & Inside Passage Discovery'",
  "subtitle": "string — budget + month like 'Mid-Range · March 2026'",
  "totalBudget": { "jpy": number, "usd": number },
  "days": [
    {
      "dayNumber": 1,
      "title": "string — day title like 'Arrive in Tokyo'",
      "destination": "string — slug like 'tokyo'",
      "items": [
        {
          "time": "string — like 'Morning' or '5:30 AM'",
          "description": "string — what to do, with personality",
          "priceJpy": number_or_null,
          "priceUsd": number_or_null,
          "category": "transport|accommodation|activity|food|ferry",
          "affiliateType": "hotel|tour|transport|null",
          "affiliateSlotId": "string_or_null",
          "locationName": "string — specific place name for map pin, e.g. 'Mendenhall Glacier', 'Homer Spit', 'Denali Visitor Center'. Use real, specific place names."
        }
      ]
    }
  ]
...
function buildUserPrompt(body: RequestBody, destinations: string, duration: string, budgetLevel: string): string {
  ...
  let prompt = `Plan a ${duration}-day Alaska trip:\n${parts.join('\n')}`;
