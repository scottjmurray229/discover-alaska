 ...
 *   2. Create project "Discover Alaska"
...
function generateMetadata(filename) {
  ...
  if (type === 'dest') {
    return {
      title: `${title} Alaska Travel Guide ${year} | Real Prices & Local Tips`,
      ...
      playlist: `${dest?.region || 'alaska'}-destinations`,
    };
  }

  if (type === 'pillar') {
    return {
      title: `${title} — Alaska ${year}`,
      description: `${title}\n\nFull guide: https://discoveralaska.info/${slug}/`,
      tags: [`Alaska ${slug}`, 'Alaska travel', `Alaska ${year}`],
      categoryId: '19',
  ...
  if (type === 'short') {
    return {
      title: `${title} Alaska ✈️ #shorts`,
      description: `${title} — Full guide: https://discoveralaska.info/destinations/${slug}/`,
      tags: [`${title} Alaska`, 'Alaska travel', 'shorts'],
      categoryId: '19',
      privacyStatus: 'private',
      playlist: 'shorts',
    };
...
function generateDestDescription(slug, title, year) {
  return `Complete travel guide to ${title}, Alaska — with real prices, how to get there, where to eat, where to stay, and what most tourists miss.

I've been exploring Alaska for years with deep local knowledge. No sponsored content.

📖 FULL WRITTEN GUIDE: https://discoveralaska.info/destinations/${slug}/
📱 TRIP COMPANION (works offline): https://discoveralaska.info/companion/

#Alaska #${title.replace(/\s+/g, '')} #AlaskaTravel #TravelGuide${year}`;
}
...
function generateDestTags(slug, title, dest) {
  const tags = [
    `${title} Alaska`,
    `${title} travel guide`,
    `Alaska ${new Date().getFullYear()}`,
    `${title} things to do`,
    `where to stay ${title}`,
    'Alaska travel',
    'Alaska food',
    'Alaska budget travel',
    'Alaska tips',
  ];
  if (dest?.region) tags.push(`${dest.region} Alaska`);
...
async function main() {
  console.log('╔══════════════════════════════════════════════════════════╗');
  console.log('║   YOUTUBE UPLOAD — DISCOVER ALASKA                 ║');
  console.log('╚══════════════════════════════════════════════════════════╝\n');
